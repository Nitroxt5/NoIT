...
from PyQt5.QtWidgets import QLabel, QVBoxLayout, QHBoxLayout, QSizePolicy, QWidget, QProgressBar

from ui.styles import button_style, dialog_background_style, progress_bar_style
...
class ProgressBar(QWidget):
    ...
    def start_typing_effect(self):
        ...
        def type_next():
            if self.typing_index >= len(self.start_text):
                self.typing_timer.stop()
                return
            self.typing_index += 1
            self.label.setText(self.start_text[:self.typing_index])
